...
function runCommand(command) {
    ...
    if (commandDet['name'] === 'ls') {
        if (!commandDet['args'][0]) {
            ...
            for (let n in commandDet['args']) {
                if (commandDet['args'][n].indexOf("/") > -1 || commandDet['args'][n].indexOf(".") > -1 || commandDet['args'][n].indexOf("~") > -1) {
                    dir_out++
            ...
            for (let i in commandDet['args']) {
                ...
                if (commandDet['args'][i].indexOf("/") > -1 || commandDet['args'][i].indexOf(".") > -1 || commandDet['args'][i].indexOf("~") > -1) {
    ...
    if (commandDet['name'] === 'uname') {
        if (!commandDet['args'][0]) {
            ...
            for (let i = 0; i < commandDet['args'].length; i++) {
                if (commandDet['args'][i] !== '-s' && commandDet['args'][i] !== '-o' && commandDet['args'][i] !== '-a' && commandDet['args'][i] !== '-m' && commandDet['args'][i] !== '-n' && commandDet['args'][i] !== '-r' && commandDet['args'][i] !== '-p' && commandDet['args'][i] !== '-v' && commandDet['args'][i] !== '-i' && commandDet['args'][i] !== '--help') {
                    return {
                        "content": "uname: invalid option -- '" + commandDet['args'][i].substr(1) + "'<br>Try 'uname --help' for more information.<br>",
                        "sta": false
                    }
                }
            }
            ...
            if (commandDet['args'][0] === '-a') {
                return {
                    "content": "AxtonOs axton.cc 7.10.0-693.2.2.el7.x86_64 #1 SMP Tue Sep 12 22:26:13 UTC 2017 x86_64 x86_64 x86_64 AxtonOs<br>",
                    "sta": true
                }
            }
    ...
    if (commandDet['name'] === 'axtonshell') {
        if (!commandDet['args'][0]) {
            return {
                "content": "Usage: axtonshell [OPTION]...<br>Set your preferences of the shell.<br><br> -b, Set the color of the background<br> -f, Set the color of the font<br> -a, Turn on/off the auto complete<br> -s, Turn on/off the soft keyboard<br> --help, Display this help and exit<br> --about, Display about infomation and exit<br>",
                "sta": true
            }
        } else {
            if (commandDet['args'][0] !== '-s' && commandDet['args'][0] !== '-l' && commandDet['args'][0] !== '-b' && commandDet['args'][0] !== '-a' && commandDet['args'][0] !== '-f' && commandDet['args'][0] !== '--help' && commandDet['args'][0] !== '--about') {
                return {
                    "content": "axtonshell: invalid option -- '" + commandDet['args'][0].substr(1) + "'<br>Try 'axtonshell --help' for more information.<br>",
                    "sta": false
                }
            }
            ...
            if (commandDet['args'][0] === '--about') {
                return {
                    "content": "<br><div><pre class=\"aslogo\">" + logo + "</pre><div class=\"about\">&nbsp;<br>Axton Shell 2.0.0.f59ed7a<br><span class=\"info\">Based on BASH<br>Written by <span class=\"js\">Javascript</span><br>Powered by <span class=\"nginx\">Nginx</span></span><br><br>Made with love by Axton</div><div class=\"clear\"></div></div><br>",
                    "sta": true
                }
            }
        }
...
function positionToObj(positionObj) {
    ...
    if (positionObj["deepth"] === 1) {
        ...
        if (positionObj["d_3"] === 0) {
            ...
            targetObj = files[0]["content"][positionObj["d_1"] - 1]["content"][positionObj["d_2"] - 1]["content"][positionObj["d_3"] - 1]
        }
    }
    return targetObj
}
function bgColor(input, source) {
    if (input !== "1" && input !== "2" && input !== "3" && input !== "4" && input !== "5") {
        ...
        if (input === "1" && fColorNow === "3") {
            ...
            fontChanged = true;
            localStorage.setItem("as-fc", "1")
        } else if ((input === "2" && fColorNow === "1") || (input === "3" && fColorNow === "1") || (input === "4" && fColorNow === "1") || (input === "5" && fColorNow === "1")) {
        ...
        let alsoUse = "";
        if (!source) {
            alsoUse = "<br>You can also use 'axtonshell -b " + input + "'."
        }
        return {
...
function skTaggle(input, source) {
    if (input !== "1" && input !== "2" && input !== "3") {
        ...
        let alsoUse = "";
        if (!source) {
            alsoUse = "<br>You can also use 'axtonshell -s " + input + "'."
        }
        return {
            "content": '<span class="as-successed">Successed.</span>' + alsoUse + '<br>',
            "sta": true
...
function sendMessage(message) {
    if (message.length > 5 && message.length < 150 && message.indexOf("<") === -1 && message.indexOf(">") === -1 && message.indexOf("\\") === -1 && message.indexOf("`") === -1 && message.indexOf("|") === -1 && message.indexOf("_") === -1 && message.indexOf(" ") !== -1) {
        $.ajax({
            url: "/send/",
            method: "POST",
            timeout: 10000,
...
function doCommand() {
    ...
    if (catMode) {
        ...
        if (input === "") {
            ...
            $('.content').html($('.content').html() + deleteHtml($('.input-text').text()) + '<br>' + deleteHtml($('.input-text').text()) + '<br>')
        }
    } else if (colorMode) {
        let input = deleteHtml($('.input-text').text().trim());
    ...
    if (!running) {
        ...
        if (command !== '') {
            ...
            if (!taggled) {
                $('.content').html($('.content').html() + '[' + username + '@axton.cc ' + displayPosition + ']#<br>')
            }
            return
        }
    } else {
...
function getReturn(firstCall) {
    ...
    if (rest["content"] !== undefined) {
        let prefix = "";
        if (firstCall) {
            prefix = '[' + username + '@axton.cc ' + displayPosition + ']# ' + deleteHtml($('.input-text').text()) + '<br>'
        }
        if (rest["content"] === '--CATMODE--') {
            ...
            let prefixSub = '';
            if (firstCall) {
                prefixSub = '[' + username + '@axton.cc ' + displayPosition + ']#' + '<br>'
            }
            ...
            let prefixSub = '';
            if (firstCall) {
                prefixSub = '[' + username + '@axton.cc ' + rest["content"].substr(10) + ']# ' + deleteHtml($('.input-text').text()) + '<br>'
            }
            $('.content').html($('.content').html() + prefixSub)
        } else if (rest["content"] === '--BGCMODE--') {
            $('.content').html($('.content').html() + prefix + 'Set your background color of the shell.<br>You can choose following colors:<br>1. <span class="bg-white bg-demo">White</span><br>2. <span class="bg-black bg-demo">Black</span><br>3. <span class="bg-dgray bg-demo">DeepGray</span><br>4. <span class="bg-dblue bg-demo">DeepBlue</span><br>5. <span class="bg-dgreen bg-demo">DeepGreen</span><br>');
            $('.prefix,.preblank').hide();
            $('.precolor').show();
            colorMode = 1
        } else if (rest["content"] === '--SKCMODE--') {
            $('.content').html($('.content').html() + prefix + 'Turn on/off the soft keyboard.<br>You have following choices:<br>1. Auto<br>2. Always on<br>3. Always off<br>');
            $('.prefix,.preblank').hide();
            $('.preoption').show();
            skMode = 1
        } else if (rest["content"] === '--FCMODE--') {
            $('.content').html($('.content').html() + prefix + 'Set your font color of the shell.<br>You can choose following colors:<br>1. <span class="fc-black fc-demo">Black</span><br>2. <span class="fc-gray fc-demo">Gray</span><br>3. <span class="fc-white fc-demo">White</span><br>');
            $('.prefix,.preblank').hide();
            $('.precolor').show();
            fcMode = 1
        } else if (rest["content"] === '--MESSAGEMODE--') {
            $('.content').html($('.content').html() + prefix + 'Leave me a message, tap Enter to send. The first word separated by space will be your name.<br>');
            $('.prefix,.preblank').hide();
            $('.preleave').show();
            lmMode = 1
        } else {
...
let tapButton = (('ontouchstart' in window) || (window.DocumentTouch && document instanceof DocumentTouch)) ? 'touchstart': 'click';
document.getElementById("enter").addEventListener(tapButton,
function() {
    prsKey({
        "keyCode": 13,
        "ctrlKey": false,
...
false);
document.getElementById("c-c").addEventListener(tapButton,
function() {
    prsKey({
        "keyCode": 67,
        "ctrlKey": true,
...
false);
document.getElementById("tab").addEventListener(tapButton,
function() {
    prsKey({
        "keyCode": 9,
        "ctrlKey": false,
...
false);
document.getElementById("up").addEventListener(tapButton,
function() {
    prsKey({
        "keyCode": 38,
        "ctrlKey": false,
...
false);
document.getElementById("down").addEventListener(tapButton,
function() {
    prsKey({
        "keyCode": 40,
        "ctrlKey": false,
...
function prsKey(event) {
    ...
    if (event.keyCode === 13) {
        ...
        if (!tabMode) {
            ...
            $('.content').html($('.content').html() + '[' + username + '@axton.cc ' + displayPosition + ']# ' + deleteHtml($(".input-text").text()) + prefixStr + '<div class="clear"></div></div>');
            $('body,html').animate({
                scrollTop: $(document).height()
            },
            0);
        ...
        if (event.keyCode === 67) {
            if (catMode) {
                ...
                $('.content').html($('.content').html() + 'Type the number of the color: ' + deleteHtml($('.input-text').text()) + '^C<br>');
                colorMode = 0;
                if (commandRunning.length === 0) {
                    running = 0
                }
                ...
                $('.content').html($('.content').html() + 'Type the number of the option: ' + deleteHtml($('.input-text').text()) + '^C<br>');
                skMode = 0;
                if (commandRunning.length === 0) {
                    running = 0
                }
                ...
                $('.content').html($('.content').html() + '[' + username + '@axton.cc ' + displayPosition + ']# ' + deleteHtml($('.input-text').text()) + '^C<br>');
                $('.input-text').text('');
                $('body,html').animate({
                    scrollTop: $(document).height()
                },
